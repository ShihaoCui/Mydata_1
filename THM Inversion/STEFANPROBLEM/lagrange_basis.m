   ...
   case 'Q8'
    ...
    if size(coord,2) < 2
      ...
      N=1/4*[(1-xi)*(1-eta)*(-xi-eta-1);
             ...
             2*(1-xi^2)*(1+eta);
             2*(1-eta^2)*(1-xi)];
      dNdxi=1/4*[-(1-eta)*(-xi-eta-1)-(1-xi)*(1-eta),-(1-xi)*(-xi-eta-1)-(1-xi)*(1-eta);
                  (1-eta)*( xi-eta-1)+(1+xi)*(1-eta),-(1+xi)*( xi-eta-1)-(1+xi)*(1-eta);
                  (1+eta)*( xi+eta-1)+(1+xi)*(1+eta), (1+xi)*( xi+eta-1)+(1+xi)*(1+eta);
                 -(1+eta)*(-xi+eta-1)-(1-xi)*(1+eta), (1-xi)*(-xi+eta-1)+(1-xi)*(1+eta);
                  2*(-2*xi)*(1-eta) , -2*(1-xi^2)        ;
                  2*(1-eta^2)       ,  2*(-2*eta)*(1+xi) ;
                  2*(-2*xi)*(1+eta) ,  2*(1-xi^2)        ;
                 -2*(1-eta^2)       ,  2*(-2*eta)*(1-xi)];
    end
   case 'Q9'
